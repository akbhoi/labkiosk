...
| **"Remote Control" opens a noVNC page that asks for a password or cannot connect** | The workstation has not reported its `vncPassword` / `remoteHost` yet (no heartbeat since boot), or it has no tunnel. | The autostart writes `/tmp/labkiosk/vnc.secret` and the agent sends it with every heartbeat; the tunnel hostname comes from `/etc/cloudflared/config.yml` (`ingress: hostname:`) or `LABKIOSK_REMOTE_HOST`. Without a per-workstation tunnel there is nothing to connect to; see README "Remote Control Prerequisites". |
